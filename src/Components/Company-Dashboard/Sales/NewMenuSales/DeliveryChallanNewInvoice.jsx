...
import DeliveryChallanNewTab from './DeliveryChallanNewTab';
...
const DeliveryChallanNewInvoice = () => {
    ...
    const mappedOrders = staticQuotations.map(quotation => {
      ...
      return {
        ...
        amount: quotation.grand_total 
          ...
          : `$ ${quotation.items?.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0).toFixed(2)}`,
  ...
  >
    <span style={{ fontWeight: 'bold', fontSize: '18px' }}>+</span> Create Delivery Challan
  </Button>
</div>
  ...
  <Modal show={stepModal} onHide={handleCloseModal} size="xl" centered>
    ...
    <Modal.Body>
        ...
        <DeliveryChallanNewTab         key={selectedOrder?.id || 'new'} // force remount on different order
...
export default DeliveryChallanNewInvoice;