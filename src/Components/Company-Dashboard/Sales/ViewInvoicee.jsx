...
import {
  ...
  FaGlobe, FaExchangeAlt, FaTimes, FaCaretUp, FaLanguage,FaArrowLeft
} from 'react-icons/fa';
...
<div className="mb-3">
  <Button 
    ...
    onClick={() => navigate('/company/Invoice')} 
    className="d-flex align-items-center gap-2"
    style={{ fontSize: '0.95rem', padding: '6px 12px' }}
  >
    <FaArrowLeft /> {languageMode === 'ar' ? 'العودة' : 'Back'}
  </Button>
</div>
      <Row className="mb-4">
      ...
      <div className="fw-bold mt-2">{getText('grossAmount')} <span className="text-success">R 10.90</span></div>
        ...
        <Col md={6}>
          <div className="table-responsive">
            <Table borderless className={languageMode === 'ar' ? 'text-end' : 'text-end'}>
              <tbody>
                <tr><td>{getText('subTotal')}</td><td>R 10.00</td></tr>
                <tr><td>{getText('tax')}</td><td>R 1.90</td></tr>
                <tr><td>{getText('shipping')}</td><td>R 0.00</td></tr>
                <tr className="fw-bold border-top"><td>{getText('total')}</td><td>R 10.90</td></tr>
                <tr className="text-danger"><td>{getText('paymentReceived')}</td><td>(-) R 10.90</td></tr>
                <tr className="fw-bold border-top"><td>{getText('balanceDue')}</td><td>R 0.00</td></tr>
              </tbody>
            </Table>
          </div>
        </Col>
